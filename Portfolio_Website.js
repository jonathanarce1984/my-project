...
// This function displays the first image in slideshow when the page loads
var slideIndex = 1;
...
function showSlides(n) {
    ...
    for (i = 0; i < slides.length; i++) {
        slides[i].style.display = "none"; // This for loop takes each time in the array "slides" amd sets the display to none
    }
    for (i = 0; i < dots.length; i++) {
        dots[i].className; dots[i].className.replace(" active", ""); // This for loop takes each item in the array "dots" and removes "active" which removes the active sytling
    }
